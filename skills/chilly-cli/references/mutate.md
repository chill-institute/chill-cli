...
- TV provider browsing is per-request. Prefer `chilly tv-shows --source <provider>` for reads; patch `catalog.tvShowsSource` only when explicitly managing persisted settings compatibility.
